function [EER, confInterEER, OP, confInterOP, DI, xx, yy, xROC, yROC]=EER_DET(clients,imposteurs,OPvalue,pas0)
%function [EER, confInterEER, OP, confInterOP, DI, h]=EER_DET_conf(clients,imposteurs,OPvalue,pas0)
...
[FARconfMIN,  FRRconfMIN , ~, ~]=ParamConfInter(FAR/100,FRR/100,num_imposteurs,num_clients);
...
% Decidability index
mgen = mean(clients);
mimp = mean(imposteurs);
vgen = var(clients);
vimp = var(imposteurs);
DI = abs(mgen-mimp)/(sqrt((vgen+vimp)/2));
...
% FAR vs FRR
figure(1);
plot (x,FRR,'-r','LineWidth',2);
hold on;plot (x,FAR,'-m','LineWidth',2);

xlabel ('Threshold');
ylabel ('Error');
title ('FAR vs FRR graph');
legend('FRR for Anon','FAR for Anon');
...
%ppndf([Pfa_min Pfa_max])
%[Pfa_min Pfa_max]
%pticks(tmin_fa:tmax_fa)
%xlabels(tmin_fa:tmax_fa,:)
